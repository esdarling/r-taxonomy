...
data %>%
  dplyr::select(contains("locality"))

# make dcd_locality from original columns
# don't use hol_locality

data %>%
  dplyr::select(verbatim_latitude, verbatim_longitude) %>%
  slice(25:30)
...
data <- data %>%
  mutate(
    location_info      = if_else(!is.na(locality), glue("{locality}"), NA_character_),
    verbatim_latitude  = clean_coord(verbatim_latitude),
    verbatim_longitude = clean_coord(verbatim_longitude),
    dcd_locality = case_when(
      !is.na(verbatim_latitude) & !is.na(location_info) ~
        glue("{location_info}, {verbatim_latitude}, {verbatim_longitude}"),
      !is.na(location_info) ~ glue("{location_info}"),
      TRUE ~ NA_character_
    )
  )

data %>% 
  dplyr::select(dcd_locality) 

data %>%
  select(verbatim_latitude, verbatim_longitude) %>%
  dplyr::slice(25:30)
...
summary_specimens <- data %>%
  select(species,
         ...
         dcd_locality,
         collector_s) %>%
  # Drop rows where key grouping fields are NA
    ...
    for (ii in type_list) {
      ...
      for (j in country_list) {
        ...
        state_list <- species_data %>%
          ...
          dplyr::filter(!is.na(state_province)) %>%
        ...
        for (k in state_list) {
          ...
          locality_specimens <- summary_specimens %>%
            ...
            dplyr::filter(!is.na(dcd_locality)) %>%
            ...
            dplyr::mutate(
              # Only include collector if not NA
              collecting_summary = if_else(
                !is.na(collector_s),
                glue::glue("{collector_s} ({specimens})"),
                glue::glue("{specimens}")
              )
            ) %>%
